...
# @Maurice @Rashid Das ist die grundlegende Bewertungslogik mit den Print befehlen 
    def process(self):
        ...
        if self.risk_com == 0:
            print("1.1: No risk communication provided. Analysis terminated.")
            return
        print("1.1: Risk communication detected.")

        # 1.2 (-> keine Ausgabe). Unterscheidung zwischen one case und two case -> bereits in Binärvariablen gespeichert
        if self.one_case == 1:
            print("1.2: Single case detected.")
        else:
            print("1.2: Two cases detected: comparing two risk scenarios.")
        ...
        if self.one_case == 0:
            if self.abs_number_diff is not None:
                if self.abs_number_base is not None and self.abs_number_new is None:
                    ...
                    print(f"2.1: Calculated absolute number (new) = {self.abs_number_new}")
                    ...
                    print(f"2.1: Calculated absolute number (base) = {self.abs_number_base}")
        ...
        if self.abs_number_base is not None and self.ref_class_size_base is not None:
            ...
            if self.abs_risk_base is None:
                ...
                print(f"2.2: Calculated absolute risk (base) = {self.abs_risk_base:.2f}%")
        if self.abs_number_new is not None and self.ref_class_size_new is not None:
            ...
            if self.abs_risk_new is None:
                ...
                print(f"2.2: Calculated absolute risk (new) = {self.abs_risk_new:.2f}%")
        ...
        if self.rel_risk is not None:
            if self.abs_risk_base is not None and self.abs_risk_new is None:
                ...
                print(f"2.3: From relative risk and base absolute risk calculated: absolute risk (new) = {self.abs_risk_new:.2f}%")
            elif self.abs_risk_new is not None and self.abs_risk_base is None:
                self.abs_risk_base = self.abs_risk_new / self.rel_risk
                print(f"2.3: From relative risk and new absolute risk calculated: absolute risk (base) = {self.abs_risk_base:.2f}%")
        ...
        if self.verbal_desc_base or self.verbal_desc_new or self.verbal_desc_change:
            ...
            print("3.2: No verbal risk descriptors present.")

        # 3.3. Check reference class descriptions
        if self.ref_class_desc_base or self.ref_class_desc_new:
            print("3.3: Reference class description present.")
        else:
            print("3.3: Warning: No reference class provided. Please check the text for an explicit description.")

        # 4. Check missing values
        print("\n--- Step 4: Missing Values Check ---")
        if self.one_case == 1:
            if self.abs_risk_base is not None:
                print("4.1: Absolute risk calculable.")
            else:
                print("4.1: No absolute risk provided or calculable – risk communication not transparent.")
        else:
            if self.abs_risk_base is not None and self.abs_risk_new is not None:
                print("4.1: Both absolute risks calculable. Risk communication transparent.")
            elif self.abs_risk_base is None and self.abs_risk_new is not None:
                print("4.1: Missing absolute risk for base case, but new absolute risk provided – risk communication not transparent.")
            elif self.abs_risk_base is not None and self.abs_risk_new is None:
                print("4.1: Base absolute risk provided, but new absolute risk missing – risk communication not transparent.")
            else:
                print("4.1: Neither absolute risk provided nor calculable – risk communication not transparent.")

        # 5. Transparent presentation to end user
        print("\n--- Step 5: Transparent Presentation ---")
        if self.abs_risk_base is not None:
            per_100_base = self.abs_risk_base
            print(f"5.1: Absolute risk (base): {self.abs_risk_base:.2f}% ({per_100_base:.2f} per 100)")
        if self.abs_risk_new is not None:
            per_100_new = self.abs_risk_new
            print(f"5.1: Absolute risk (new): {self.abs_risk_new:.2f}% ({per_100_new:.2f} per 100)")
        if self.rel_risk is not None:
            change_pct = abs((self.rel_risk - 1) * 100)
            if self.rel_risk < 1:
                print(f"5.2: The risk in the new case is {change_pct:.2f}% lower than in the base case.")
            elif self.rel_risk > 1:
                print(f"5.2: The risk in the new case is {change_pct:.2f}% higher than in the base case.")
            else:
